...
pub fn zk_prove_plain_proof(
    ...
    // Convert PlainProof to proof parameters (legacy seeds; callers that need V3
    // must pass SeedDerivation via a dedicated entry point when added).
    let (private, public) = proof.parse_proof(block_header, SeedDerivation::Legacy)?;
    if sanity_check {
