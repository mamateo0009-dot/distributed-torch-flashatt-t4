// MMA accumulator lane partition for SIMT and TensorOp int8 GemmShape<128,128,32> / WarpShape<32,64,32>.
// SIMT: FragmentC is 64 cells as a Cartesian 8x8 hash tile, but spatially interleaved as four
// 4x4 blocks (row stride 16, col stride 32) — LaneMmaShape<4,4> + Policy WarpShape<4,8>.
// TensorOp: 4x16 periodic hash tile on Turing Sm75 Tensor Cores.
// Case 9: 4 warps in M x 2 warps in N.
...
struct MmaTensorOpLaneTile128x128 {
  static constexpr int kThreadblockM = 128;
  static constexpr int kThreadblockN = 128;
  static constexpr int kThreadsPerCta = 256;
  static constexpr int kCellsPerThread = 64;
  static constexpr int kHashH = 4;
  static constexpr int kHashW = 16;
  static constexpr int kWarpsM = 4;
  static constexpr int kWarpsN = 2;
...
#define MMA_LANE_TILE_FN inline
  ...
  /* Origin (min row, min col) of this thread's 4x16 periodic hash tile within the CTA. */
  MMA_LANE_TILE_FN static void thread_block_origin(int thread_idx, int& row0,
                                                   int& col0) {
    int const warp = thread_idx / 32;
    int const lane = thread_idx % 32;
    ...
    row0 = warp_m * 32 + (lane / 4);
    col0 = warp_n * 64 + (lane % 4) * 2;
  }
