...
pub struct CachedStats {
    ...
    pub cached_json: RwLock<Option<bytes::Bytes>>,
}
...
pub struct AppState {
    ...
    pub latest_job_id: Arc<RwLock<Option<String>>>,
    pub current_block_height: Arc<AtomicU64>,
    pub cached_stats: Arc<CachedStats>,
}
...
impl AppState {
    pub fn new(
        ...
        Self {
            ...
            latest_job_id: Arc::new(RwLock::new(None)),
            current_block_height: Arc::new(AtomicU64::new(0)),
            cached_stats: Arc::new(CachedStats::new()),
        }
    }

    pub fn update_worker_seen(&self, worker_id: &str, ip: &str) {
        let now = chrono::Utc::now().timestamp();
        if let Some(mut entry) = self.workers_stats.get_mut(worker_id) {
            entry.last_seen = now;
            if !ip.is_empty() && entry.ip != ip {
                entry.ip = ip.to_string();
            }
        } else {
            self.workers_stats.insert(
                worker_id.to_string(),
                WorkerStats {
    ...
    pub fn record_share(&self, worker_id: &str, accepted: bool, hs: f64, job_id: &str) {
        if accepted {
    ...
    pub fn get_stats(&self) -> ProxyStatsResponse {
        ...
        let current_job_id = self.latest_job_id.read().clone();
        let current_block_height = {
            let h = self.current_block_height.load(Ordering::Relaxed);
            if h > 0 {
                Some(h)
            } else {
                None
    ...
    pub fn get_cached_stats_json(&self) -> bytes::Bytes {
        let now_ms = chrono::Utc::now().timestamp_millis();
        let cached_time = self.cached_stats.timestamp_ms.load(Ordering::Relaxed);

        if now_ms - cached_time < 1000 {
            if let Some(ref b) = *self.cached_stats.cached_json.read() {
                return b.clone();
            }
        }

        let stats = self.get_stats();
        let json_bytes = bytes::Bytes::from(serde_json::to_vec(&stats).unwrap_or_default());

        *self.cached_stats.cached_json.write() = Some(json_bytes.clone());
        self.cached_stats.timestamp_ms.store(now_ms, Ordering::Relaxed);
...
mod tests {
    ...
    #[test]
    fn test_record_share_and_stats_caching() {
        ...
        state.record_share("miner-1", true, 15_000_000_000_000.0, "job-1");
        state.record_share("miner-1", false, 15_000_000_000_000.0, "job-2");

        let stats = state.get_stats();
        assert_eq!(stats.total_shares_accepted, 1);
        assert_eq!(stats.total_shares_rejected, 1);
        assert_eq!(stats.workers.len(), 1);
        assert_eq!(stats.workers[0].shares_accepted, 1);
        assert_eq!(stats.workers[0].shares_rejected, 1);

        let json1 = state.get_cached_stats_json();
        assert!(!json1.is_empty());
        let json2 = state.get_cached_stats_json();
